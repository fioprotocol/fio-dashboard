...
const RefFlow = (
  ...
  const isRefSite = IS_REFERRAL_PROFILE_PATH.test(pathname);
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated) {
      // Set refProfileCode to cookies from ref site
      if (refProfileInfo?.code != null) {
        ...
        // Update refProfileCode cookies and set ref profile. Works for non auth user and non ref site
        if (!isRefSite) {
          const cookieRefProfileCode =
            Cookies.get(REFERRAL_PROFILE_COOKIE_NAME) || null;

          getInfo(cookieRefProfileCode);
          setCookies(REFERRAL_PROFILE_COOKIE_NAME, cookieRefProfileCode, {
            expires: REFERRAL_PROFILE_COOKIE_EXPIRATION_PEROID,
          });
        }
      }
    }
  }, [refProfileInfo?.code, isAuthenticated, isRefSite, getInfo]);

  useEffect(() => {
    // load profile for ref site
    if (isRefSite && !isAuthenticated) {
      const refProfileCode = pathname.split('/')[2];
      getInfo(refProfileCode);
    }
  }, [isRefSite, pathname, isAuthenticated, getInfo]);
