...
- Run script should be like `"REACT_APP_WIDGET=${REACT_APP_WIDGET} npm run client:start"`.
- Build script should be like `"REACT_APP_WIDGET=${REACT_APP_WIDGET} npm run client:build && source ./bin/build-widget.sh"`.

Before build:
- install jq package `https://stedolan.github.io/jq/`

How to build embed widget?
- Set `WIDGET_PUBLIC_URL` and `REACT_APP_WIDGET` NODE env vars.
- Set `WIDGET_PUBLIC_URL` in `client/.env` file. If you want to test it locally set full path to widget's build folder.
